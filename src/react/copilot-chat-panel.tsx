...
export function CopilotChatPanel(props: CopilotChatPanelProps): ReactElement {
  ...
  const hasDiagnostics =
    errorIssues.length > 0 || warnIssues.length > 0 || sectionHandlerMissing;
  ...
  return (
    ...
    >
      ...
      <CardContent className="flex min-h-0 flex-1 flex-col gap-2 p-0">
        <ScrollArea data-testid="copilot-thread" className="min-h-0 flex-1">
          <div className="flex min-h-full flex-col gap-2 px-2">
            ...
            {toolCalls.length > 0 ? (
              <div data-testid="copilot-tool-calls" className="flex flex-col">
                <AnimatePresence initial={false}>
                  {toolCalls.map((toolCall) => (
                    <CopilotToolCallRow key={toolCall.id} toolCall={toolCall} />
                  ))}
                </AnimatePresence>
              </div>
            ) : null}
