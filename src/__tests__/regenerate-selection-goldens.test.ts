...
describe("regenerateSelection goldens (11 demo components)", () => {
	...
	it.each(
		componentNames.map((name) => [name] as const),
	)("%s — mock generator output matches the locked patch", async (name) => {
	...
	it.each(
		componentNames.map((name) => [name] as const),
	)("%s — locked patch passes validateAiSectionPatch against the same context", (name) => {
		const ctx = buildContext(name);
